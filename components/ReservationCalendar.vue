...
const props = defineProps<{ dates: string[]; slots: Slot[]; bookings: Booking[]; userId: string | null; pendingSlotId: number | null }>()
...
<template><div class="calendar"><section v-for="date in props.dates" :key="date" class="day"><h2>{{ title(date) }}</h2><div class="slots"><ReservationSlot v-for="slot in props.slots" :key="slot.id" :time-slot="slot" :booking="bookingFor(date, slot.id)" :is-mine="bookingFor(date, slot.id)?.userId === props.userId" :pending="props.pendingSlotId === slot.id" @reserve="emit('reserve', slot, date)" @cancel="emit('cancel', $event)" /></div></section></div></template>
