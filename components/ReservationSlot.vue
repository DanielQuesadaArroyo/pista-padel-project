...
<template>
  <button
    ...
    :class="{
      expired,
      occupied: !expired && props.booking && !props.isMine,
      mine: !expired && props.booking && props.isMine,
    }"
    :disabled="props.pending || expired || Boolean(props.booking && !props.isMine)"
    type="button"
    @click="selectSlot"
  >
...
.slot.expired {
  border-color: var(--color-slot-expired);
  background: var(--color-slot-expired);
  color: var(--color-text-primary);
}
.slot.occupied {
  border-color: var(--color-slot-occupied);
  background: var(--color-slot-occupied);
  color: #fff;
}
.slot.mine {
  border-color: var(--color-slot-mine);
  background: var(--color-slot-mine);
  color: #fff;
}
.slot:disabled {
  cursor: not-allowed;
